...
1. 从 [Amazon Quick 下载页](https://aws.amazon.com/quick/download/) 下载并安装 Desktop 版本。（国内下载链接：https://amzn-chn.feishu.cn/drive/folder/NimEfRYqPltk9edER33caQmMnpg）
...
- 整理需求、约束和需要确认的事项。
- 将需求拆分为带依赖关系的实施单元。
- 根据依赖关系安排执行顺序和可并行处理的工作。
- 检查计划结构，并将实施计划保存为 Markdown 文件。

### 6.3 输出

Plan 完成后，实施计划会显示在右侧 **Session tabs** 中。

实施计划会根据 PRD 的范围拆分为若干实施单元。每个单元会说明目标、依赖关系、预期产物和验证方式；具体单元数量和组合以本次生成结果为准。
...
- 读取并检查实施计划中的任务和依赖关系。
- 按计划执行当前可以开始的实施单元。
- 收集各实施单元的结果，并处理执行中出现的问题。
- 生成并检查计划要求的最终产物；本 Hands-on 的最终产物为 `SKILL.md`。
- 完成后将结果交给用户确认和保存。
...
如果 Quick 没有主动询问，可在当前 Chat 中输入：

```text
使用 save_skill 工具把最终生成的 Skill 保存到我的技能库中。
```
